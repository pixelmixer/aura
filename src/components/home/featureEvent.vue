        ...
        <v-layout wrap align-center justify-center row fill-height class="mt-2 elevation-2 white" style="border:1px solid #e0e0e0;border-radius:5px">
           ...
           <v-flex xs12 sm8 md9 lg9 class="pa-2 py-4 px-3" >
                ...
                <span class="google-font mt-1 mb-0 grey--text"  style="font-size:105%">
                    ...
                    {{eventDetails.EventDate && eventDetails.EventDate.Date +'/'+ eventDetails.EventDate.Month +'/'+ eventDetails.EventDate.Year}}
                    &nbsp;
                    <v-icon small>watch_later</v-icon>
                    {{eventDetails.EventTime && eventDetails.EventTime.StartTime +' - '+ eventDetails.EventTime.EndTime}}
                    &nbsp;
               ...
               <p class="google-font mt-2 mb-1" style="font-size:115%;color:#757575">
                ...
                <v-btn color="#1a73e8" v-if="eventDetails.RegistrationLink && eventDetails.RegistrationLink.length>0" :href="eventDetails.RegistrationLink" target="_blank" class="ma-0 elevation-0 my-2" dark style="text-transform: capitalize;border-radius:5px;">
                    Registration Link
                </v-btn>
                &nbsp;
...
export default {
    ...
    filters:{
        summery: (val,num)=>{
            return val && val.substring(0,num)+".."
        },
        dateFilter: (value)=>{
          if(!value) return '';
